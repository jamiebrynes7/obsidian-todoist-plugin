...
<div class="month-controls">
  <span class="control-button" on:click={decrementMonth}><svg
  ...
  <span
    class="current-month">{moment(`${year}-${month + 1}-01 00:00:00`).format('MMM YYYY')}</span>
  <span class="control-button" on:click={incrementMonth}><svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 20 20"
      fill="currentColor">
      <path
  ...
  {#each monthData as week}
    <div class="week">
      {#each week as day}
        {#if day}
          <div
            class="day {selected?.isSame(day, 'day') ? 'selected-day' : ''} {moment().isAfter(day, 'day') ? 'past-day' : ''}"
